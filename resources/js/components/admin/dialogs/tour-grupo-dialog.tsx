...
import type { Option, TourGrupo, TourGrupoTipo } from '@/types/admin';

const TIPOS: { value: TourGrupoTipo; label: string }[] = [
...
export const TOUR_GRUPO_TIPO_LABEL: Record<TourGrupoTipo, string> = {
    cabecalho: 'Cabeçalho',
    destaque: 'Destaque',
...
export default function TourGrupoDialog({
    item,
    onClose,
}: {
    item: TourGrupo | null;
    ...
    const { data, setData, post, put, processing, errors } = useForm({
        tipo: (item?.tipo ?? 'destaque') as TourGrupoTipo,
        titulo: item?.titulo ?? '',
    ...
    const submit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (item) {
            put(`/admin/tours-grupos/${item.id}`, options);
        } else {
            post('/admin/tours-grupos', options);
        }
    };
        ...
        >
            <Field id="tipo" label="Tipo" error={errors.tipo}>
                <Select
                    value={data.tipo}
                    onValueChange={(value) =>
                        setData('tipo', value as TourGrupoTipo)
                    }
                >
                ...
                <Field
                    ...
                    label={
                        data.tipo === 'cta_whatsapp'
                            ? 'Número (WhatsApp) — vazio usa o número global'
                            : 'Email'
                    }
                    error={errors.link}
                >
